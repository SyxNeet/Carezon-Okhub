...
/**
 * Section: About — Restaurant Page
 * ACF Group: restaurant_about
 */

$about = get_field('restaurant_about') ?: [];
$label       = $about['label'] ?? 'Về chúng tôi ?';
$description = $about['description'] ?? '';
$paragraphs  = $about['body_paragraphs'] ?? [];
$images      = $about['images'] ?? [];

// Fallback static assets
$d_main_img    = get_theme_file_uri('/assets/images/restaurant-page/d-about-main.png');
// Decorative food composition — exported flat từ Figma node "Frame 2147264368"
$d_deco_group  = get_theme_file_uri('/assets/images/restaurant-page/d-about-deco-group.png');

// Nav arrow icon (vector)
$ic_arrow = get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group1.svg');

// Lotus decorative SVGs
$lotus_svgs = [
    get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group1.svg'),
    get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group2.svg'),
    get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group3.svg'),
    get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group4.svg'),
    get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group5.svg'),
    get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group6.svg'),
    get_theme_file_uri('/assets/images/restaurant-page/d-lotus-group7.svg'),
];

// Build carousel image list from ACF Gallery or fallback to static images
$carousel_images = [];
if (! empty($images)) {
    ...
    // Fallback: static main image
    $carousel_images[] = [
        'id'  => 0,
        'alt' => 'Carezone Restaurant',
        'url' => $d_main_img,
    ];
}
?>
...
<section id="section-about" class="section-about">
    ...
    <div class="section-about__bg-gradient" aria-hidden="true"></div>

    <!-- Lotus decoration (desktop only) -->
    <div class="section-about__lotus" aria-hidden="true">
        <?php
        $lotus_classes = [
            'section-about__lotus-g1',
            'section-about__lotus-g2',
            'section-about__lotus-g3',
            'section-about__lotus-g4',
            'section-about__lotus-g5',
            'section-about__lotus-g6',
            'section-about__lotus-g7',
        ];
        foreach ($lotus_svgs as $idx => $svg_url) :
            $cls = $lotus_classes[$idx] ?? '';
        ?>
            <div class="<?php echo esc_attr($cls); ?>">
                <img src="<?php echo esc_url($svg_url); ?>" alt="" aria-hidden="true" loading="lazy">
            </div>
        <?php endforeach; ?>
    </div>
        ...
        <div class="section-about__inner">
            ...
            <div class="section-about__label-col">
                <div class="section-about__label">
                    <span class="section-about__label-dot" aria-hidden="true"></span>
            ...
            <div class="section-about__content-col">
                ...
                <div class="section-about__text-block">
                    <div class="section-about__text-group">
                        ...
                        <?php if ($description) : ?>
                            <h2 class="section-about__description">
                        ...
                        <?php else : ?>
                            <p class="section-about__paragraph">
                                Mỗi món ăn tại đây đều được chế biến tinh tế, giữ trọn dưỡng chất và hương vị tươi ngon từ rau củ, ngũ cốc và thảo mộc tự nhiên. Không chỉ giúp cơ thể nhẹ nhàng mà còn thanh lọc tâm hồn, mang lại trải nghiệm sống lành mạnh và cân bằng.
                            </p>
                            <p class="section-about__paragraph">
                                Mỗi món ăn tại đây đều được chế biến tinh tế, giữ trọn dưỡng chất và hương vị tươi ngon từ rau củ, ngũ cốc và thảo mộc tự nhiên. Không chỉ giúp cơ thể nhẹ nhàng mà còn thanh lọc tâm hồn, mang lại trải nghiệm sống lành mạnh và cân bằng.
                            </p>
                    ...
                    <div class="section-about__nav">
                        <button
                            ...
                            type="button"
                            aria-label="<?php esc_attr_e('Ảnh trước', 'okhub-theme'); ?>">
                        ...
                        <button
                            ...
                            type="button"
                            aria-label="<?php esc_attr_e('Ảnh tiếp theo', 'okhub-theme'); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none">
                    ...
                    </div><!-- /.section-about__nav -->

                </div><!-- /.section-about__text-block -->

                <!-- Image carousel (Swiper) -->
                <div
                    class="section-about__carousel swiper"
                    data-total="<?php echo esc_attr(count($carousel_images)); ?>">
                    <div class="section-about__carousel-track swiper-wrapper">
                        <?php foreach ($carousel_images as $i => $img) : ?>
                            <div class="section-about__carousel-slide swiper-slide">
                                <?php if (! empty($img['id'])) : ?>
                                    <?php echo wp_get_attachment_image($img['id'], 'large', false, [
                                        'class'    => 'section-about__carousel-img',
                                        'loading'  => $i === 0 ? 'eager' : 'lazy',
                                        'decoding' => 'async',
                                        'alt'      => esc_attr($img['alt'] ?: 'Carezone Restaurant'),
                                    ]); ?>
                                <?php else : ?>
                                    <img
                                        src="<?php echo esc_url($img['url']); ?>"
                                        alt="<?php echo esc_attr($img['alt'] ?: 'Carezone Restaurant'); ?>"
                                        class="section-about__carousel-img"
                                        width="546"
                                        height="546"
                        ...
                        <?php endforeach; ?>
                    </div><!-- /.section-about__carousel-track -->
                </div><!-- /.section-about__carousel -->

            </div><!-- /.section-about__content-col -->

        </div><!-- /.section-about__inner -->
    </div><!-- /.section-about__container -->

    <!-- Decorative food composition group — 1 ảnh phẳng export từ Figma -->
    <div class="section-about__deco-group" aria-hidden="true">
        <img
            src="<?php echo esc_url($d_deco_group); ?>"
            alt=""
            aria-hidden="true"
