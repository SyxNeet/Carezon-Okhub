<?php
$section_therapy_packages = get_field('section_therapy_packages') ?: [];

$section_title = $section_therapy_packages['title'] ?? '';

$packages = $section_therapy_packages['packages'] ?? [];

$package_large = $section_therapy_packages['package_large'] ?? [];
$package_large_title = $package_large['title'] ?? '';
$package_large_bg = $package_large['background_image'] ?? null;
$package_large_bg_mobile = $package_large['background_image_mobile'] ?? null;
$package_large_features = $package_large['package_features'] ?? [];

$member_section = $section_therapy_packages['member_section'] ?? [];
$member_section_title = $member_section['title'] ?? '';
$member_features = $member_section['member_feature'] ?? [];

$bg_id = wp_is_mobile() ? 1440 : 1441;
$bg_section_url = wp_get_attachment_image_url($bg_id, 'full');
$bg_combo_card = 1439;
$bg_combo_card_mb = 1438;
$bg_member_card = 1198;
...
<section class="therapy-member-section-wrapper">
    <div class="therapy-member-section" style="background-image: url('<?php echo $bg_section_url; ?>');">
        <div class="section-container">
            <div class="combo-wrapper">
                <h2 class="section-heading"><?= esc_html($section_title); ?></h2>

                <div class="combo-grid">
                    <?php foreach ($packages as $package):
                        $package_title = $package['package_title'] ?? '';
                        $package_description = $package['package_description'] ?? '';
                        $background_image = $package['background_image'] ?? null;
                        $package_features = $package['package_features'] ?? [];
                        ?>

                        <div class="combo-card">
                            <?= wp_get_attachment_image($bg_combo_card, 'full', false, array('class' => 'card-bg')) ?>
                            <?= wp_get_attachment_image($bg_combo_card_mb, 'full', false, array('class' => 'card-bg mobile')) ?>
                            <div class="card-inner">
                                <div class="card-text">

                                    <h3 class="card-label"><?= esc_html($package_title); ?></h3>
                                    <p class="card-desc"><?= esc_html($package_description); ?></p>
                                    ...
                                    <?php if (!empty($package_features)): ?>
                                        <ul class="benefit-list">
                                            <?php foreach ($package_features as $feature):
                                                ...
                                                ?>
                                                <li>
                                                    <div class="benifit-item-icon">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="11" height="9"
                                                            viewBox="0 0 11 9" fill="none">
                                                            <path d="M0.436523 4.66641L2.95895 7.18883L9.71126 0.436523"
                                                                stroke="#2D6635" stroke-width="1.23528" />
                                                        </svg>
                                                    </div>
                                                    <span><?= esc_html($feature_value); ?></span>
                                                </li>
                                            <?php endforeach; ?>
                                        </ul>
                                    <?php endif; ?>
                                ...
                                <?php echo wp_get_attachment_image($background_image, 'full', false, array('class' => 'person-img')); ?>
                            </div>
                        </div>
                ...
                <div class="combo-wide-card">

                    <?= wp_get_attachment_image($package_large_bg, 'full', false, array('class' => 'combo-wide-card-bg')) ?>
                    <?= wp_get_attachment_image($package_large_bg_mobile, 'full', false, array('class' => 'combo-wide-card-bg mobile')) ?>

                    <div class="wide-card-overlay"></div>

                    <div class="wide-card-content">
                        <h3 class="card-label"><?= esc_html($package_large_title); ?></h3>
                        <?php if (!empty($package_large_features)): ?>
                            <ul class="benefit-list">
                                <?php foreach ($package_large_features as $feature):
                                    ...
                                    <li>
                                        <div class="benifit-item-icon">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="11" height="9" viewBox="0 0 11 9"
                                                fill="none">
                                                <path d="M0.436523 4.66641L2.95895 7.18883L9.71126 0.436523" stroke="#2D6635"
            ...
            <div class="member-wrapper">
                <h2 class="section-heading no-text-gradient"><?= esc_html($member_section_title); ?></h2>

                <div class="member-slider-container">
                    <div class="swiper member-swiper">
                        <div class="swiper-wrapper">
                            <?php foreach ($member_features as $item):
                                ...
                                <div class="swiper-slide">
                                    <div class="member-card">
                                        <?= wp_get_attachment_image($bg_member_card, 'full', false, array('class' => 'member-card-bg')); ?>
                                        <div class="member-card-body">
                                            <h4 class="member-card-title"><?= esc_html($title); ?></h4>
                                            <?php if (!empty($feature_values)): ?>
                                                <ul class="member-benefit-list">
                                                    <?php foreach ($feature_values as $feature):
                                                        $value = $feature['value'] ?? '';
                                                        if (!$value) {
                                                            continue;
                                                        }
                                                        ?>
                                                        <li>
                                                            <div class="member-card-item-icon">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="11" height="9"
                                                                    viewBox="0 0 11 9" fill="none">
                                                                    <path d="M0.4375 4.66665L2.95992 7.18908L9.71223 0.436768"
                                                                        stroke="white" stroke-width="1.23528" />
                                                                </svg>
                                                            </div>
                    ...
                    <?php if (count($member_features) >= 4): ?>
                        <div class="member-nav-btn member-prev">
                        ...
                        <div class="member-nav-btn member-next">
                            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none">
                                <path
